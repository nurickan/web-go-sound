...
func NewSVF(sr float64) *SVF { return &SVF{sr: sr, h: 1.0 / (1.0 + 2.0*0.5 + 1.0)} }

func (f *SVF) SetCutoff(freq float64) {
	freq = Clamp(freq, 20.0, Nyquist)
	g := tan(freq * 3.14159265 / f.sr)
	f.g = g; f.h = 1.0 / (1.0 + 2.0*f.r*g + g*g)
}

func (f *SVF) SetResonance(res float64) { f.r = Clamp(res, 0.0, 1.0) }
func (f *SVF) SetMode(m SVFMode) { f.mode = m }

func (f *SVF) Process(in, out MonoBuffer) {
	for i := range in { out[i] = f.tick(in[i]) }
}
...
func (f *SVF) Reset() { f.ic1eq = 0; f.ic2eq = 0 }
